...
class Book extends Controller
{
    ...
    function getBookbyTheme($theme)
    {
        ...
        $stmt = $conn->prepare("SELECT count(*) as total_row FROM book WHERE theme = ?");
        ...
        //retrieve the selected results from database   
        $stmt = $conn->prepare ("SELECT * FROM book LIMIT " . $page_first_result . ',' . $results_per_page . " WHERE theme = ?");  
        $stmt->bind_param("s", $theme);
        $stmt->execute();
        $result = $stmt->get_result();
