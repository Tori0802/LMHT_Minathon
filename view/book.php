...
?>
    ...
    <div class="flex-shrink-0 m-3 p-3 bg-light text-dark bg-opacity-50" style="width: calc(120px + 2.5%); box-sizing: border-box;">
        ...
        <section class="mx-3 py-2 col-9">
            ...
            <?php 
                $type = "SELECT DISTINCT theme FROM book";
                $listtype = mysqli_query($data['connection'], $type);
        
                if (mysqli_num_rows($listtype) > 0) {
                    foreach ($listtype as $type) {
            ?>
            
            <h1 class="text-primary fw-semibold my-5"><?php echo $type['theme'];?></h1> 
            <div class="container px-3 px-lg-5 mt-5">
                <div class="row gx-4 gx-lg-5 row-cols-3 row-cols-md-3 row-cols-xl-4 justify-content-start">
                    ...
                    $list = mysqli_query($data['connection'], $book);
                    if (mysqli_num_rows($list) > 0) {
                        foreach ($list as $book) {
                            if ($book['theme'] == $type['theme']) {
                    ?>
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                                ...
                                <div class="text-center p-1">
                                    <form action="/bookstore/book/getDetail" method="GET">
                                        <button type='submit' class="btn btn-outline-primary" name = 'id' value = <?php echo $book['id'] ?>>About product</button>
                                    </form>
                                </div>
                            </div>
                        </div>
